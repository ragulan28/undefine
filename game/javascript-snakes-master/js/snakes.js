...
Snakes = function () {
    ...
    var cfg = {
        ...
        keys: [
            {
                keys: [Game.Key.Y, Game.Key.Q], mode: 'down', state: 'quit', action: function () {
                this.quitGame();
            }
            },
            {
                keys: [Game.Key.N, Game.Key.ESC], mode: 'down', state: 'quit', action: function () {
                this.continueGame();
            }
            },
            {
                keys: [Game.Key.ESC, Game.Key.RETURN], mode: 'down', state: 'highscores', action: function () {
                this.backToMenu();
            }
            },
            {
                keys: [Game.Key.ESC, Game.Key.RETURN], mode: 'down', state: 'credits', action: function () {
                this.backToMenu();
            }
            },
            {
                keys: [Game.Key.ESC, Game.Key.RETURN], mode: 'down', state: 'name', action: function () {
                this.saveHighScore();
            }
            },
            {
                keys: [Game.Key.LEFT, Game.Key.A], mode: 'down', state: 'game', action: function () {
                this.snake.move(DIR.LEFT);
            }
            },
            {
                keys: [Game.Key.RIGHT, Game.Key.D], mode: 'down', state: 'game', action: function () {
                this.snake.move(DIR.RIGHT);
            }
            },
            {
                keys: [Game.Key.UP, Game.Key.W], mode: 'down', state: 'game', action: function () {
                this.snake.move(DIR.UP);
            }
            },
            {
                keys: [Game.Key.DOWN, Game.Key.S], mode: 'down', state: 'game', action: function () {
                this.snake.move(DIR.DOWN);
            }
            },
            {
                key: Game.Key.ESC, mode: 'down', state: 'game', action: function () {
                this.quitGame();
            }
            }
        ],
    ...
    var game = Class.create({
        ...
        onentercredits   : function (event, from, to) {
            this.dom.credits.fadein();
        },
        onleavecredits   : function (event, from, to) {
        ...
        unoccupied: function (pos) {
            var max = 100, p = pos || new Game.Point(), p1 = new Game.Point(), p2 = new Game.Point(),
                p3 = new Game.Point(), p4 = new Game.Point();
            while (--max) {
        ...
        }, // safari sucks at playing short sound FX (delays)
        isMute         : function () {
        ...
        buildMenu: function () {
            ...
            return new Game.Menu(this.dom.overlay, this, {
                ...
                items   : [
                    {
                        text: 'New Game', title: "Start a new game of snakes...", action: function () {
                        this.newGame();
                    }
                    },
                    {
                        title: "Change Difficulty level", action: function (d) {
                        this.resetDifficulty(d);
                    }, choice: difficulties, chosen: this.storage.difficulty
                    },
                    {
                        text: 'High Scores', title: "View the high score table", action: function () {
                        this.viewScores();
                    }
                    },
                    {
                        text: 'Credits', title: "Thanks to the games contributors", action: function () {
                        this.viewCredits();
                    }
                    },
                ]
        ...
        buildQuitMenu: function () {
            return new Game.Menu(this.dom.overlay, this, {
                ...
                items   : [
                    {
                        text: 'Abandon', title: "Abandon the current game (Y/N) ?", action: function () {
                        this.quitGame();
                    }
                    },
                    {
                        text: 'Continue', title: "Continue the current game ?", action: function () {
                        this.continueGame();
                    }
                    }
                ]
    ...
    var score = Class.create({
        ...
        buildHighScoreTable: function (data, editEntry) {
            ...
            if (is.valid(this.game.storage.highscores) && !editEntry) {
                list.append({
                    tag: 'li',
                    dom: $({tag: 'span', html: 'reset', klass: 'reset', on: {click: this.onreset.bind(this)}})
                });
            }
        },
    ...
    var sparkles = Class.create({
        ...
        add: function (pos, options) {
            ...
            spark.animator.addSubject(function (value) {
